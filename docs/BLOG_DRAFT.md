...
4. The agent got back a plain-text denial (`[ATC-DENIED] reason=denied_by_human
   ...
   error, just a normal tool result it could actually reason about.
...
anything, the pending card already read `blast_radius: '~200 rows affected'` — a real
...
prior state captured before it runs —
...
would even fire — SigNoz's default evaluation window is around a minute — a runaway loop (or, as
it turns out, three perfectly well-behaved agents doing nothing wrong) has already spent the
money. The breaker checks cumulative spend
...
- **SQLite, not the Postgres sitting right there.** We provisioned and seeded a real Postgres
  ...
  runs on its SQLite fallback because the Postgres backend never actually got built.
...
panel is actually built on, only carries `agent_id`, `risk`, and `decision` as labels. `rule_id`
